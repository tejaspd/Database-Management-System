...
class CommandInterpreter {
    ...
    void loadStatement() {
        _input.next("load");
        String newName = name();
        Table _table = Table.readTable(newName);
        _database.put(newName, _table);
        _input.next(";");
        System.out.printf("Loaded %s.db%n", newName);
    }
    ...
    void printStatement() {
        _input.next("print");
        String newName = _input.peek();
        Table table = tableName();
        System.out.printf("Contents of %s:%n", newName);
        _input.next(";");
        table.print();
    }
    ...
    Table selectClause() {
        ...
        if (_input.nextIf(",")) {
            ...
            newTable = table.select(secondTable, list, conditions);
        } else {
            conditions = conditionClause(table);
            newTable = table.select(list, conditions);
        }
        return newTable;
    }
     ...
     *  token stream. pass in the elements of the condition */
    Condition condition(Table... tables) {
        ...
        if (_input.nextIs(Tokenizer.IDENTIFIER)) {
            ...
            cond = new Condition(c, newRelation, newC);
        } else {
            cond = new Condition(c, newRelation, literal());
        }
        return cond;
    }
